...
本文件定义 `wasteland2` 第一版可实现、可验证的最小玩法规格，用于指导：
...
`MVP_SPEC.md` 只描述“游戏必须如何表现”，不规定具体代码架构或开发阶段拆分。
...
- `claimed` job 必须绑定所需 reservation，防止多个角色占用同一唯一目标
- `tree`、`blueprint`、`bed` 这类唯一 object 同一时刻只能被一个角色保留用于当前 job
- `itemPile` 在 MVP 中按整 pile 保留，不做按数量拆分 reservation
- `pickup` / `eat` 需要保留对应的 `itemPile`
- `deliver` / `construct` 需要保留对应的 `blueprint`
- job 完成、失败、取消、被高优先级需求打断时，相关 reservation 必须释放
- 若 reservation 对应目标消失、不可达或已不再满足前置条件，job 必须进入 `failed` 或 `cancelled`
...
- 选择一个格子或对象
- 对 tree 下达 `chop tree`
- 选择 `wall` 或 `bed` 并放置 blueprint
- 对未完成的 blueprint 下达 `cancel blueprint`
- 控制游戏速度：`pause` / `1x` / `2x`
...
- 当前地图内容
- 角色当前位置
- tree / wall / bed / blueprint / itemPile 的基础区分
- 当前总资源信息，至少包含 `wood` 与 `meal`
- 选中对象的基本信息
- 调试信息，至少包含 tick、角色需求值、active job
...
实现阶段允许先用 `1` 名角色打通单人闭环，但默认验证世界必须回到 `2 ~ 3` 名角色以检查并发稳定性。
...
- 角色释放 active job
- 若手中有材料，材料掉回地图
- 系统不保留脏 claim 与脏 reservation

### 12.7 多角色并发稳定性

- 地图上同时存在至少 `2` 个可执行普通工作
- `2 ~ 3` 名角色不会 claim 同一唯一目标
- 同一 `itemPile` 不会被重复搬运
